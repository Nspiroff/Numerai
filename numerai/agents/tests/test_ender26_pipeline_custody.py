...
class TestEnder26PipelineCustody(unittest.TestCase):
    @staticmethod
    def _experiment(root: Path) -> Path:
        return root / pipeline._ENDER26_PREFIX
    ...
    def _custody(self, root: Path, name: str):
        ...
        external = {
            ...
            for relative in pipeline._ENDER26_EXTERNAL_ARTIFACTS
        }
        manifest = {
            "schema_version": 1,
            "git_head": "a" * 40,
            "files": {
                ...
                for relative in pipeline._ENDER26_ROUND1_MANIFEST_FILES
            },
        ...
        authority_relative = (
            f"{pipeline._ENDER26_PREFIX}/protocol/discovery_data_authority.json"
        )
        for relative in pipeline._ENDER26_ROUND1_MANIFEST_FILES:
            leases.append(
                Lease(
        ...
        custody = pipeline._Ender26BootstrapCustody(
            round_number=1,
            config_name=name,
            manifest=manifest,
    ...
    def test_manifest_and_governed_output_sets_are_exact(self) -> None:
        self.assertEqual(len(pipeline._ENDER26_ROUND1_MANIFEST_FILES), 31)
        self.assertEqual(
            pipeline._ENDER26_EXTERNAL_ARTIFACTS,
            {
                "numerai/v5.3/ender21_discovery_full_through_0861.parquet",
                (
                    "numerai/v5.3/"
        ...
        self.assertEqual(
            pipeline._ENDER26_DISCOVERY_AUTHORITY,
            "ender26-discovery-only",
        )
        required = {
            ...
            f"{pipeline._ENDER26_PREFIX}/experiment.md",
            f"{pipeline._ENDER26_PREFIX}/gate.md",
            f"{pipeline._ENDER26_PREFIX}/protocol/discovery_data_authority.json",
            f"{pipeline._ENDER26_PREFIX}/configs/base_r1.py",
            *(
                f"{pipeline._ENDER26_PREFIX}/configs/{name}.py"
                for name in pipeline._ENDER26_ROUND1_NAMES
            ),
            f"{pipeline._ENDER26_PREFIX}/run_round1.py",
            f"{pipeline._ENDER26_PREFIX}/training_bootstrap.py",
            f"{pipeline._ENDER26_PREFIX}/evaluation_common.py",
            f"{pipeline._ENDER26_PREFIX}/evaluate_round1.py",
            f"{pipeline._ENDER26_PREFIX}/evaluate_round1_impl.py",
        ...
        self.assertEqual(pipeline._ENDER26_ROUND1_MANIFEST_FILES, required)

        experiment = Path(pipeline.REPO_DIR) / pipeline._ENDER26_PREFIX
        expected_outputs = {
            *(
            ...
            *(
            ...
            experiment / "receipts/round1_bmc_alignment.json",
        }
        governed = pipeline._governed_output_paths()
        self.assertTrue(expected_outputs.issubset(governed))
        self.assertFalse(
            any(
                pipeline._ENDER26_PREFIX in str(path) and "round2" in str(path).lower()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(pipeline, "REPO_DIR", root):
                for name in pipeline._ENDER26_ROUND1_NAMES:
                    ...
                    self.assertEqual(
                        pipeline._ender26_config_identity(config, None),
                        (1, name),
                    )
                base = self._config(root, "base_r1")
                with self.assertRaisesRegex(ValueError, "frozen named Ender26"):
                    pipeline._ender26_config_identity(base, None)
                round2 = self._config(root, "r2_grank_resid_seed7331")
                with self.assertRaisesRegex(ValueError, "frozen named Ender26"):
                    pipeline._ender26_config_identity(round2, None)
                config = self._config(root, pipeline._ENDER26_ROUND1_NAMES[0])
                with self.assertRaisesRegex(ValueError, "may not be redirected"):
                    pipeline._ender26_config_identity(config, root / "bypass")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = {
                ...
                "output": {
                    ...
                    "results_name": pipeline._ENDER26_ROUND1_NAMES[0],
                },
            }
            with mock.patch.object(pipeline, "REPO_DIR", root), mock.patch.object(
    ...
    def test_create_new_triple_has_exact_paths_and_rejects_reuse(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            name = pipeline._ENDER26_ROUND1_NAMES[0]
            config = self._config(root, name)
            with mock.patch.object(pipeline, "REPO_DIR", root):
                reservations = pipeline._ender26_output_reservations(config, None)
                ...
                second = pipeline._ender26_output_reservations(config, None)
                assert second is not None
                with self.assertRaisesRegex(ValueError, "Cannot reserve exclusive"):
                    with second:
                        self.fail("CREATE_NEW unexpectedly reused Ender26 outputs")
    ...
    def test_custody_requires_exact_held_envelope_and_authority(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                with mock.patch.object(pipeline, "REPO_DIR", root):
                    self.assertIs(
                        pipeline._validate_ender26_bootstrap_custody(
                            (1, name), custody
                        ),
                        custody,
                    )
                    shortened = pipeline._Ender26BootstrapCustody(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                with mock.patch.object(
                    ...
                    pipeline,
                    "_validate_ender26_bootstrap_custody",
                    ...
                    pipeline, "_write_ender26_completion", side_effect=completion
                ):
                    outputs = pipeline.run_training(
                        config, _ender26_bootstrap_custody=custody
                    )
    ...
    def test_bootstrap_entry_binds_exact_round_and_custody(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(pipeline, "REPO_DIR", root), mock.patch.object(
                ...
                outputs = pipeline.run_ender26_training_from_bootstrap(
                    config,
                    round_number=1,
            ...
            custody = runner.call_args.kwargs["_ender26_bootstrap_custody"]
    ...
    def test_round1_completion_hash_binds_reserved_handles(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in pipeline._ENDER26_ROUND1_NAMES:
                ...
                with mock.patch.object(pipeline, "REPO_DIR", root):
                    with pipeline._ExclusiveOutputReservations(
                        ...
                        path, payload, payload_bytes = (
                            pipeline._write_ender26_completion(
                ...
                self.assertEqual(
                    payload["stage"], "ender26-round1-training-completion"
                )
