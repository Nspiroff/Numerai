...
EXPERIMENT = (
    ...
    / "experiments/ender27_tempered_gaussian_rank_residual_v53"
)
...
COMMON = _load_module("ender27_evaluation_common_test", EXPERIMENT / "evaluation_common.py")
BOOTSTRAP = _load_module(
    "ender27_training_bootstrap_evaluator_test",
...
try:
    ROUND1 = _load_module(
        "ender27_evaluate_round1_test", EXPERIMENT / "evaluate_round1_impl.py"
    )
finally:
...
class TestEnder27PreflightAndCustody(unittest.TestCase):
    ...
    def test_failed_decision_is_terminal_zero_byte_create_new_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "round1_tempered_alignment.json"
...
class TestEnder27FrozenContracts(unittest.TestCase):
        ...
        for name in COMMON.ROUND1_NAMES:
            with self.subTest(name=name):
                ...
                if "grank" in name:
                    ...
                    self.assertEqual(
                        procedure["benchmark_transform_strength"], 0.5
                    )
                    self.assertEqual(
                        transform["benchmark_transform_strength"], 0.5
                    )
                else:
                    ...
                    self.assertNotIn("benchmark_transform_strength", transform)
...
class TestEnder27DecisionRules(unittest.TestCase):
...
class TestEnder27DecisionEnvelope(unittest.TestCase):
    @staticmethod
    def _frame():
        return pd.DataFrame(
            {
    ...
    def _records():
        values = {
            ...
            "r1_tempered_grank_resid_seed1337": _metrics(full=0.0103, recent=0.0125),
            "r1_control_rawresid_seed2027": _metrics(full=0.0100, recent=0.0120),
            "r1_tempered_grank_resid_seed2027": _metrics(full=0.0102, recent=0.0121),
        }
        return {
            name: {
                "metrics": metrics,
    ...
    def test_positive_is_source_eligible_but_authorizes_nothing(self) -> None:
        ...
        self.assertEqual(payload["stage"], "ender27-round1-tempered-alignment")
        self.assertEqual(payload["state"], "ENDER27_ROUND2_SOURCE_GATE_ELIGIBLE")
        ...
        self.assertEqual(
            ...
            [
                ...
                "model.target_transform.benchmark_transform_strength",
                "output.results_name",
            ],
        )
    ...
    def test_any_gate_failure_is_exact_terminal_negative(self) -> None:
        ...
        records["r1_tempered_grank_resid_seed2027"]["metrics"]["corr_mean"] = 0.04
        payload = self._evaluate(records)
        self.assertFalse(payload["passed"])
        self.assertFalse(payload["round2_source_gate_eligible"])
        self.assertEqual(
            payload["state"], "ENDER27_NEGATIVE_NO_TEMPERED_ALIGNMENT_GAIN"
        )

    def test_pair_cohort_or_provenance_mismatch_fails_closed(self) -> None:
        records = self._records()
        records["r1_tempered_grank_resid_seed1337"]["provenance"] = {
