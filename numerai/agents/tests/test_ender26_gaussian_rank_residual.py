...
EXPERIMENT = REPO / "numerai/agents/experiments/ender26_gaussian_rank_residual_v53"
...
BOOTSTRAP = _load_module(
    "ender26_training_bootstrap_source_test", EXPERIMENT / "training_bootstrap.py"
)
COMMON = _load_module(
    "ender26_evaluation_common_source_test", EXPERIMENT / "evaluation_common.py"
)


class TestEnder26FrozenSource(unittest.TestCase):
    def test_exact_four_run_cohort_and_matched_pair_delta(self) -> None:
        ...
        self.assertEqual(tuple(pipeline._ENDER26_ROUND1_NAMES), expected)
    ...
    def test_manifest_inventory_is_exact_and_equal_everywhere(self) -> None:
        ...
        production = set(pipeline._ENDER26_ROUND1_MANIFEST_FILES)
        self.assertEqual(len(bootstrap), 31)
        self.assertEqual(bootstrap, evaluator)
        self.assertEqual(bootstrap, production)
        self.assertEqual(set(BOOTSTRAP.EXTERNALS), set(pipeline._ENDER26_EXTERNAL_ARTIFACTS))
    ...
    def test_source_freeze_has_only_round1_and_no_artifacts(self) -> None:
        ...
        self.assertIn("ENDER26_ROUND2_SOURCE_GATE_ELIGIBLE", text)
        self.assertIn("ENDER26_NEGATIVE_NO_BMC_ALIGNMENT_GAIN", text)
        self.assertIn("y_c - m * (m.T @ y_c) / (m.T @ m)", text)


class TestEnder26SequentialBootstrap(unittest.TestCase):
    @staticmethod
    ...
    def test_preexisting_future_or_decision_path_stops_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            future = (
                experiment
                / "results/r1_grank_resid_seed2027.json"
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            decision = (
                root
                / BOOTSTRAP.PREFIX
                / "receipts/round1_bmc_alignment.json"
            )
    ...
    def test_valid_predecessor_is_held_and_allows_next_pipeline_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            pipeline = SimpleNamespace(
                run_ender26_training_from_bootstrap=mock.Mock(return_value=None)
            )
            with mock.patch.object(BOOTSTRAP, "REPO_DIR", root), mock.patch.object(
                BOOTSTRAP, "_require_launch"
            ), mock.patch.object(
            ...
            pipeline.run_ender26_training_from_bootstrap.assert_called_once()
