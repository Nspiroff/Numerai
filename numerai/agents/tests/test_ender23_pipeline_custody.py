...
class TestEnder23PipelineCustody(unittest.TestCase):
    @staticmethod
    def _experiment(root: Path) -> Path:
        return root / "numerai/agents/experiments/ender23_temporal_retention_v53"
    ...
    def test_direct_generic_cli_rejected_before_output_reservation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config = self._config(root, pipeline._ENDER23_ROUND1_NAMES[0])
    ...
    def test_reservation_precedes_manifest_config_and_data(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            name = pipeline._ENDER23_ROUND1_NAMES[0]
            ...
            with mock.patch.object(pipeline, "REPO_DIR", root), mock.patch.object(
                pipeline, "_validate_ender23_bootstrap_custody", side_effect=validate
            ), mock.patch.object(
                pipeline,
                ...
                with self.assertRaisesRegex(AssertionError, "CONFIG_EVALUATED"):
                    pipeline.run_training(
                        config, _ender23_bootstrap_custody=custody
                    )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = {
                ...
                "output": {
                    ...
                    "results_name": pipeline._ENDER23_ROUND1_NAMES[0],
                },
            }
            with mock.patch.object(pipeline, "REPO_DIR", root), mock.patch.object(
    ...
    def test_canonical_config_rejects_output_override_before_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = self._config(root, pipeline._ENDER23_ROUND1_NAMES[0])
            with mock.patch.object(pipeline, "REPO_DIR", root), mock.patch.object(
                pipeline, "_verify_ender23_round1_manifest"
    ...
    def test_bootstrap_entry_binds_exact_round_and_custody(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(pipeline, "REPO_DIR", root), mock.patch.object(
                ...
                outputs = pipeline.run_ender23_training_from_bootstrap(
                    config,
                    round_number=1,
            ...
            custody = runner.call_args.kwargs["_ender23_bootstrap_custody"]
            self.assertEqual(custody.round_number, 1)
            self.assertEqual(custody.config_name, name)
            self.assertEqual(
    ...
    def test_every_r1_r2_completion_hash_binds_reserved_handles(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in pipeline._ENDER23_TRAINING_NAMES:
                round_number = 1 if name in pipeline._ENDER23_ROUND1_NAMES else 2
                ...
                with mock.patch.object(pipeline, "REPO_DIR", root):
                    with pipeline._ExclusiveOutputReservations(
                        ...
                        path, payload, payload_bytes = pipeline._write_ender23_completion(
                ...
                self.assertEqual(
                    payload["stage"],
                    f"ender23-round{round_number}-training-completion",
                )
