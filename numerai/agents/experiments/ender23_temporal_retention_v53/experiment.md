# Ender23 temporal-retention experiment
...
Ender23 is a new matched experiment cohort after the training data loader's
memory-path repair. Ender22 produced no valid Round-1 decision because its
half-life-52 run failed before fitting when eager benchmark filtering attempted
an additional 3.17 GiB frame allocation. The repaired loader and its semantic
regression must be committed and bound by a new Ender23 source manifest before
any launch; this scaffold does not itself prove that precondition.

No Ender22 prediction, result, completion receipt, score, or decision is an
input to Ender23. Round 1 must rerun A, B, and C from scratch, once each, under
the same repaired code and new Ender23 Round-1 manifest. A standalone retry of
B, or mixing fresh B with Ender22 A/C artifacts, is forbidden. The research
procedures, gates, seeds, discovery boundary, and possible Round-2 pairs remain
otherwise unchanged from the predeclared Ender22 protocol.
...
confirmation Ender23 may ever use is a prospective stream of resolved eras
...
All three runs are one indivisible matched cohort. Evaluation is forbidden
until all three new Ender23 completion receipts validate under the same
Round-1 manifest; a failed or missing run terminates that cohort without a
selection.
