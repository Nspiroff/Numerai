"""Shared, fail-closed scoring rules for the frozen Ender23 experiment.
...
def verify_governed_manifest(custody: "EvaluationCustody") -> dict:
    ...
    verified = (
        _pipeline._verify_ender23_round1_manifest()
        if custody.round_number == 1
        else _pipeline._verify_ender23_round2_manifest()
    )
...
EXPERIMENT_NAME = "ender23_temporal_retention_v53"
...
def require_frozen_python_runtime(repo_dir: Path) -> Path:
    ...
    if (
        ...
        raise ValueError(
            "Ender23 evaluation requires Python -B with an isolated "
    ...
    if prefix != Path(os.path.abspath(launch)) or not prefix.is_absolute():
        raise ValueError("Ender23 pycache_prefix differs from frozen launch state")
    try:
        prefix.relative_to(Path(os.path.abspath(repo_dir)))
    except ValueError:
        pass
    else:
        raise ValueError("Ender23 pycache_prefix must be outside the repository")
    require_plain_directory_chain(prefix)
    if next(prefix.iterdir(), None) is not None:
        raise ValueError("Ender23 pycache_prefix must be freshly empty")
    return prefix
...
class EvaluationCustody:
    ...
    def _acquire(self):
        if self.round_number not in {1, 2}:
            raise ValueError("Unknown Ender23 evaluation round")
        require_frozen_python_runtime(self.repo_dir)
        prefix = Path("numerai/agents/experiments/ender23_temporal_retention_v53")
        ...
        if not isinstance(external, dict) or set(external) != expected_external:
            raise ValueError("Ender23 external artifact set differs")
        for relative, expected in sorted(files.items()):
            ...
            if lease.sha256() != expected:
                raise ValueError(f"Ender23 source hash drifted: {relative}")
        for relative, expected in sorted(external.items()):
            ...
            if (
                ...
                raise ValueError(f"Ender23 external artifact drifted: {relative}")
        ...
        if self.round_number == 2:
            ...
            if (
                ...
                or round1.get("stage") != "ender23-round1-discovery"
    ...
    def load_config(self, path: Path) -> dict:
        ...
        base_namespace: dict[str, object] = {
            ...
            "__name__": "__ender23_frozen_base__",
        }
...
def load_authority(
    ...
    if (
        ...
        or authority.get("authority") != "ender23-discovery-only"
        or authority.get("forbidden_historical_confirmation") != {
            "first_era": "0865",
            "last_era": "1021",
            "rule": "never read, score, select, tune, or report in Ender23",
        }
        ...
        raise ValueError("Ender23 discovery authority envelope differs from freeze")
    expected = {
        "full": (
...
def expected_procedure(name: str) -> dict:
    ...
    if name not in mapping:
        raise ValueError(f"Unknown Ender23 component {name}")
...
def validate_config(
    ...
    if config.get("output") != {
        "output_dir": "experiments/ender23_temporal_retention_v53",
        "results_name": name,
    }:
...
def validate_completion(
    ...
    if (
        ...
        or payload["stage"] != f"ender23-round{round_number}-training-completion"
