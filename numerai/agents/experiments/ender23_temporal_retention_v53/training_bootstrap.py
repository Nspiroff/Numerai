"""Stdlib-only custody boundary for one exact Ender23 training process."""
...
PREFIX = "numerai/agents/experiments/ender23_temporal_retention_v53"
ROUND_NAMES = {
    1: frozenset({
...
def _expected_manifest_files(round_number: int) -> frozenset[str]:
    files = {
        ...
        *(
            f"{PREFIX}/configs/{name}.py"
            for names in ROUND_NAMES.values()
            for name in names
        ),
    }
    if round_number == 2:
...
def _require_round2_selection(config_name: str, leases: tuple["_Lease", ...]) -> None:
    expected_path = Path(
        os.path.abspath(REPO_DIR / PREFIX / "receipts/round1_discovery.json")
    )
    matches = [
        lease
        for lease in leases
        if Path(os.path.abspath(lease.path)) == expected_path
    ]
    if len(matches) != 1:
        raise ValueError("Ender23 bootstrap lacks one held Round-1 decision.")
    try:
        decision = json.loads(matches[0].read_bytes())
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("Ender23 Round-1 decision is not valid JSON.") from error
    ...
    if (
        ...
        or config_name not in ROUND2_BY_SELECTED[selected]
    ):
        raise ValueError(
            "Ender23 Round-1 decision does not authorize this Round-2 config."
        )
...
def _plain_directory_chain(path: Path) -> None:
    ...
    for directory in reversed([absolute, *absolute.parents]):
        ...
        if directory.is_symlink() or bool(attributes & getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0)):
            raise ValueError(f"Ender23 bootstrap directory is a reparse point: {directory}")
        if not stat.S_ISDIR(inspected.st_mode):
            raise ValueError(f"Ender23 bootstrap directory is not plain: {directory}")
...
def _plain_file(path: Path, label: str) -> None:
    ...
    if (
        ...
        raise ValueError(f"Ender23 bootstrap {label} is not a unique plain file: {path}")
    ...
    def __init__(self, path: Path, label: str) -> None:
        ...
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(f"Cannot lease Ender23 {label}: {path}") from ctypes.WinError(ctypes.get_last_error())
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    f"Leased Ender23 {label} identity is malformed or changed."
                )
...
class _DecisionReservation:
    ...
    def __enter__(self):
        ...
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot reserve Ender23 evaluation decision: {self.path}"
        ...
        if (
            ...
            raise ValueError("Reserved Ender23 evaluation decision is malformed.")
        return self
    ...
    def commit_json(self, payload: dict) -> bytes:
        ...
        opened = os.fstat(self.stream.fileno())
        lexical = self.path.lstat()
        if self.identity != (int(opened.st_dev), int(opened.st_ino)) or self.identity != (
            int(lexical.st_dev), int(lexical.st_ino)
        ):
...
class _TrainingOutputReservations:
    ...
    def _open(path: Path, label: str):
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot reserve Ender23 {label}: {path}"
                ) from ctypes.WinError(ctypes.get_last_error())
            descriptor = msvcrt.open_osfhandle(
                int(handle), os.O_RDWR | getattr(os, "O_BINARY", 0)
            )
    ...
    def __exit__(self, *_args) -> None:
        for attribute in (
            "completion_stream", "results_stream", "predictions_stream"
        ):
    ...
    def _identity(self, label: str, path: Path, stream) -> dict:
        ...
        if (
            ...
            raise ValueError(f"Reserved Ender23 {label} identity changed.")
        return {
            "path": str(path),
    ...
    def completion_identities(self) -> dict[str, dict[str, object]]:
        ...
        for label, path, stream in (
            ...
            if inspected.st_size <= 0:
                raise ValueError(f"Completed Ender23 {label} is empty.")
            digest = hashlib.sha256()
            stream.seek(0)
    ...
    def write_completion(self, payload: dict) -> bytes:
        if os.fstat(self.completion_stream.fileno()).st_size != 0:
            raise ValueError("Reserved Ender23 completion is no longer empty.")
        payload_bytes = (
...
def _require_launch() -> Path:
    ...
    if preloaded:
        raise ValueError(f"Ender23 governed modules were pre-imported: {preloaded}")
    ...
    if (
        ...
        raise ValueError("Ender23 bootstrap requires isolated Python -I -B.")
    if type(option) is not str or not option or sys.pycache_prefix != option:
        raise ValueError("Ender23 bootstrap requires exact -X pycache_prefix.")
    prefix = Path(option)
    if not prefix.is_absolute() or prefix != Path(os.path.abspath(prefix)):
        raise ValueError("Ender23 pycache prefix must be absolute and canonical.")
    try:
        prefix.relative_to(REPO_DIR)
    except ValueError:
        pass
    else:
        raise ValueError("Ender23 pycache prefix must be outside the repository.")
    _plain_directory_chain(prefix)
    if next(prefix.iterdir(), None) is not None:
        raise ValueError("Ender23 pycache prefix must be freshly empty.")
    return prefix


def _git(*args: str, allow_one: bool = False) -> subprocess.CompletedProcess:
    result = subprocess.run(["git", *args], cwd=REPO_DIR, capture_output=True, text=True, check=False)
    if result.returncode not in ({0, 1} if allow_one else {0}):
        raise ValueError(result.stderr.strip() or "Ender23 bootstrap Git verification failed.")
    return result


def _acquire(round_number: int) -> tuple[dict, bytes, tuple[_Lease, ...]]:
    filename = f"source_manifest_round{round_number}.json"
    ...
    try:
        ...
        if not isinstance(manifest, dict) or set(manifest) != {
            ...
            raise ValueError("Ender23 bootstrap manifest schema differs.")
        if manifest["schema_version"] != 1 or manifest["hash_algorithm"] != "sha256":
            raise ValueError("Ender23 bootstrap manifest version differs.")
        ...
        if not isinstance(files, dict) or set(files) != required:
            raise ValueError("Ender23 bootstrap manifest file set differs.")
        if not isinstance(external, dict) or set(external) != EXTERNALS:
            raise ValueError("Ender23 bootstrap external set differs.")
        if manifest["runtime"] != EXPECTED_RUNTIME or platform.python_version() != EXPECTED_RUNTIME["python"]:
            raise ValueError("Ender23 bootstrap runtime differs.")
        for package, version in EXPECTED_RUNTIME["packages"].items():
            if importlib_metadata.version(package) != version:
                raise ValueError(f"Ender23 bootstrap package drifted: {package}")
        commit = manifest["git_head"]
        if not isinstance(commit, str) or len(commit) != 40 or any(c not in "0123456789abcdef" for c in commit):
            raise ValueError("Ender23 bootstrap git_head is malformed.")
        if _git("rev-parse", "--verify", f"{commit}^{{commit}}").stdout.strip() != commit:
            raise ValueError("Ender23 bootstrap git_head does not resolve.")
        if _git("merge-base", "--is-ancestor", commit, "HEAD", allow_one=True).returncode:
            raise ValueError("Ender23 bootstrap git_head is not an ancestor of HEAD.")
        ...
        if _git("status", "--porcelain=v1", "--untracked-files=all", "--", *governed).stdout:
            raise ValueError("Ender23 bootstrap source is not committed and clean.")
        for relative_text, expected in sorted(files.items()):
            ...
            if lease.sha256() != expected:
                raise ValueError(f"Ender23 bootstrap source drifted: {relative_text}")
            if _git("diff", "--quiet", commit, "--", relative_text, allow_one=True).returncode:
                raise ValueError(f"Ender23 bootstrap source differs from git_head: {relative_text}")
        for relative_text, receipt in sorted(external.items()):
            ...
            leases.append(lease)
            if lease.sha256() != receipt.get("sha256") or lease.size_bytes() != receipt.get("size_bytes"):
                raise ValueError(f"Ender23 bootstrap artifact drifted: {relative_text}")
...
def _training_main(arguments: list[str]) -> int:
    ...
    parser.add_argument("config", choices=sorted(ROUND_NAMES[1] | ROUND_NAMES[2]))
    args = parser.parse_args(arguments)
    if args.config not in ROUND_NAMES[args.round]:
        raise ValueError("Ender23 config does not belong to the requested round.")
    _require_launch()
    ...
    with _TrainingOutputReservations(args.config) as reservations:
        ...
        try:
            if args.round == 2:
                _require_round2_selection(args.config, leases)
            ...
            pipeline.run_ender23_training_from_bootstrap(
                config,
...
def _load_governed_module(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise ValueError(f"Cannot load governed Ender23 module: {path}")
...
def _evaluation_main(arguments: list[str]) -> int:
    ...
    expected_output = experiment / (
        ...
        if args.round == 1
        else "receipts/round2_seed_replication.json"
    )
    if experiment != expected_experiment:
        raise ValueError("Ender23 evaluator requires the canonical experiment.")
    if numerai_dir != Path(os.path.abspath(REPO_DIR / "numerai")):
        raise ValueError("Ender23 evaluator requires the canonical data root.")
    if output != expected_output:
        raise ValueError("Ender23 evaluator output path differs from the freeze.")
    ...
    with _DecisionReservation(output) as decision:
        ...
        try:
            ...
            implementation = _load_governed_module(
                f"ender23_evaluate_round{args.round}_impl",
                REPO_DIR / PREFIX / f"evaluate_round{args.round}_impl.py",
            )
