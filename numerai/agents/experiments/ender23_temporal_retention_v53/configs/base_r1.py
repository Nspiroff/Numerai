"""Frozen shared configuration for Ender23 temporal-retention research."""
...
def variant(
    ...
    *,
    ...
    """Return one of the protocol's mechanically bounded variants."""
    config = deepcopy(CONFIG)
    config["output"]["results_name"] = name
    config["model"]["params"]["seed"] = model_seed
    config["training"]["sample_seed"] = sample_seed
    if recency_half_life_eras is not None:
        config["model"]["params"]["recency_half_life_eras"] = (
            recency_half_life_eras
        )
    if max_train_eras is not None:
        config["training"]["cv"]["max_train_eras"] = max_train_eras
    return config
