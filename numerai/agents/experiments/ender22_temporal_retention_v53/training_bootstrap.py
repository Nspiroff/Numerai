"""Stdlib-only custody boundary for one exact Ender22 training process."""
...
def _require_round2_selection(config_name: str, leases: tuple["_Lease", ...]) -> None:
    ...
    if len(matches) != 1:
        raise ValueError("Ender22 bootstrap lacks one held Round-1 decision.")
    try:
        decision = json.loads(matches[0].read_bytes())
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("Ender22 Round-1 decision is not valid JSON.") from error
    ...
    if (
        ...
        raise ValueError(
            "Ender22 Round-1 decision does not authorize this Round-2 config."
        )
...
def _plain_directory_chain(path: Path) -> None:
    ...
    for directory in reversed([absolute, *absolute.parents]):
        ...
        if directory.is_symlink() or bool(attributes & getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0)):
            raise ValueError(f"Ender22 bootstrap directory is a reparse point: {directory}")
        if not stat.S_ISDIR(inspected.st_mode):
            raise ValueError(f"Ender22 bootstrap directory is not plain: {directory}")
...
def _plain_file(path: Path, label: str) -> None:
    ...
    if (
        ...
        raise ValueError(f"Ender22 bootstrap {label} is not a unique plain file: {path}")
    ...
    def __init__(self, path: Path, label: str) -> None:
        ...
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(f"Cannot lease Ender22 {label}: {path}") from ctypes.WinError(ctypes.get_last_error())
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    f"Leased Ender22 {label} identity is malformed or changed."
                )
...
class _DecisionReservation:
    ...
    def __enter__(self):
        ...
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot reserve Ender22 evaluation decision: {self.path}"
        ...
        if (
            ...
            raise ValueError("Reserved Ender22 evaluation decision is malformed.")
        return self
...
class _TrainingOutputReservations:
    ...
    def _open(path: Path, label: str):
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot reserve Ender22 {label}: {path}"
                ) from ctypes.WinError(ctypes.get_last_error())
            descriptor = msvcrt.open_osfhandle(
                int(handle), os.O_RDWR | getattr(os, "O_BINARY", 0)
            )
    ...
    def _identity(self, label: str, path: Path, stream) -> dict:
        ...
        if (
            ...
            raise ValueError(f"Reserved Ender22 {label} identity changed.")
        return {
            "path": str(path),
    ...
    def completion_identities(self) -> dict[str, dict[str, object]]:
        ...
        for label, path, stream in (
            ...
            if inspected.st_size <= 0:
                raise ValueError(f"Completed Ender22 {label} is empty.")
            digest = hashlib.sha256()
            stream.seek(0)
    ...
    def write_completion(self, payload: dict) -> bytes:
        if os.fstat(self.completion_stream.fileno()).st_size != 0:
            raise ValueError("Reserved Ender22 completion is no longer empty.")
        payload_bytes = (
...
def _require_launch() -> Path:
    ...
    if preloaded:
        raise ValueError(f"Ender22 governed modules were pre-imported: {preloaded}")
    ...
    if (
        ...
        raise ValueError("Ender22 bootstrap requires isolated Python -I -B.")
    if type(option) is not str or not option or sys.pycache_prefix != option:
        raise ValueError("Ender22 bootstrap requires exact -X pycache_prefix.")
    prefix = Path(option)
    if not prefix.is_absolute() or prefix != Path(os.path.abspath(prefix)):
        raise ValueError("Ender22 pycache prefix must be absolute and canonical.")
    try:
        prefix.relative_to(REPO_DIR)
    except ValueError:
        pass
    else:
        raise ValueError("Ender22 pycache prefix must be outside the repository.")
    _plain_directory_chain(prefix)
    if next(prefix.iterdir(), None) is not None:
        raise ValueError("Ender22 pycache prefix must be freshly empty.")
    return prefix
...
def _git(*args: str, allow_one: bool = False) -> subprocess.CompletedProcess:
    ...
    if result.returncode not in ({0, 1} if allow_one else {0}):
        raise ValueError(result.stderr.strip() or "Ender22 bootstrap Git verification failed.")
    return result
...
def _acquire(round_number: int) -> tuple[dict, bytes, tuple[_Lease, ...]]:
    ...
    try:
        ...
        if not isinstance(manifest, dict) or set(manifest) != {
            ...
            raise ValueError("Ender22 bootstrap manifest schema differs.")
        if manifest["schema_version"] != 1 or manifest["hash_algorithm"] != "sha256":
            raise ValueError("Ender22 bootstrap manifest version differs.")
        ...
        if _git("status", "--porcelain=v1", "--untracked-files=all", "--", *governed).stdout:
            raise ValueError("Ender22 bootstrap source is not committed and clean.")
        for relative_text, expected in sorted(files.items()):
            ...
            if lease.sha256() != expected:
                raise ValueError(f"Ender22 bootstrap source drifted: {relative_text}")
            if _git("diff", "--quiet", commit, "--", relative_text, allow_one=True).returncode:
                raise ValueError(f"Ender22 bootstrap source differs from git_head: {relative_text}")
        for relative_text, receipt in sorted(external.items()):
            ...
            if lease.sha256() != receipt.get("sha256") or lease.size_bytes() != receipt.get("size_bytes"):
                raise ValueError(f"Ender22 bootstrap artifact drifted: {relative_text}")
...
def _training_main(arguments: list[str]) -> int:
    ...
    if args.config not in ROUND_NAMES[args.round]:
        raise ValueError("Ender22 config does not belong to the requested round.")
    _require_launch()
    ...
    with _TrainingOutputReservations(args.config) as reservations:
        ...
        try:
            ...
            pipeline.run_ender22_training_from_bootstrap(
                config,
...
def _load_governed_module(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise ValueError(f"Cannot load governed Ender22 module: {path}")
...
def _evaluation_main(arguments: list[str]) -> int:
    ...
    if experiment != expected_experiment:
        raise ValueError("Ender22 evaluator requires the canonical experiment.")
    if numerai_dir != Path(os.path.abspath(REPO_DIR / "numerai")):
        raise ValueError("Ender22 evaluator requires the canonical data root.")
    if output != expected_output:
        raise ValueError("Ender22 evaluator output path differs from the freeze.")
    ...
    with _DecisionReservation(output) as decision:
        ...
        try:
            ...
            implementation = _load_governed_module(
                f"ender22_evaluate_round{args.round}_impl",
