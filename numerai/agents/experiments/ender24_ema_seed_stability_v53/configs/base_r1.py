"""Frozen shared configuration for Ender24 EMA seed-stability research."""
...
CONFIG = {
    ...
    "output": {
        "output_dir": "experiments/ender24_ema_seed_stability_v53",
...
def variant(
    ...
    *,
    ...
    """Return one exact matched control or EMA configuration."""
    config = deepcopy(CONFIG)
    config["output"]["results_name"] = name
    config["model"]["params"]["seed"] = model_seed
    if ema_decay is not None:
        config["model"]["params"]["ema_decay"] = ema_decay
    return config
