"""Shared, fail-closed scoring rules for frozen Ender27 Round 1.
...
EXPERIMENT_NAME = "ender27_tempered_gaussian_rank_residual_v53"
...
PAIR_NAMES = {
    "1337": ("r1_control_rawresid_seed1337", "r1_tempered_grank_resid_seed1337"),
    "2027": ("r1_control_rawresid_seed2027", "r1_tempered_grank_resid_seed2027"),
}
RECENT_BLOCKS = {
...
def verify_governed_manifest(custody: "EvaluationCustody") -> dict:
    ...
    if not isinstance(manifest, dict) or set(manifest) != {
        ...
        raise ValueError("Held Ender27 source manifest schema differs")
    if manifest["schema_version"] != 1 or manifest["hash_algorithm"] != "sha256":
        raise ValueError("Held Ender27 source manifest version differs")
    if set(manifest["files"]) != _manifest_file_set(1):
        raise ValueError("Held Ender27 source manifest file set differs")
    expected_external = {
    ...
    try:
        ...
        raise ValueError("Held Ender27 source manifest bytes are invalid") from error
    if decoded != manifest or raw_manifest != custody.manifest_bytes:
        raise ValueError("Held Ender27 source manifest bytes changed")
    for relative, expected in sorted(manifest["files"].items()):
        ...
        if custody.lease(custody.repo_dir / relative).sha256() != expected:
            raise ValueError(f"Held Ender27 source changed: {relative}")
    for relative, expected in sorted(manifest["external_artifacts"].items()):
        ...
        if (
            ...
            raise ValueError(f"Held Ender27 external changed: {relative}")
    return manifest
...
def require_frozen_python_runtime(repo_dir: Path) -> Path:
    ...
    if (
        ...
        raise ValueError(
            "Ender27 evaluation requires Python -B with an isolated "
    ...
    if prefix != Path(os.path.abspath(launch)) or not prefix.is_absolute():
        raise ValueError("Ender27 pycache_prefix differs from frozen launch state")
    try:
        prefix.relative_to(Path(os.path.abspath(repo_dir)))
    except ValueError:
        pass
    else:
        raise ValueError("Ender27 pycache_prefix must be outside the repository")
    require_plain_directory_chain(prefix)
    if next(prefix.iterdir(), None) is not None:
        raise ValueError("Ender27 pycache_prefix must be freshly empty")
    return prefix


def _manifest_file_set(_round_number: int = 1) -> set[str]:
    if _round_number != 1:
        raise ValueError("Ender27 currently defines only a Round-1 manifest")
    shared = {
...
class EvaluationCustody:
    """Held immutable envelope for one Ender27 Round-1 evaluation."""
    ...
    def _acquire(self):
        if self.round_number != 1:
            raise ValueError("Unknown Ender27 evaluation round")
        ...
        if not isinstance(self.manifest, dict):
            raise ValueError("Bootstrap did not supply the held Ender27 manifest")
        if not isinstance(self.manifest_bytes, bytes) or not self.manifest_bytes:
            raise ValueError("Bootstrap did not supply held Ender27 manifest bytes")
        files = self.manifest.get("files")
        external = self.manifest.get("external_artifacts")
        if not isinstance(files, dict) or not isinstance(external, dict):
            raise ValueError("Bootstrap Ender27 manifest collections differ")
        expected_paths = {
        ...
        if len(borrowed) != len(self.bootstrap_leases) or set(borrowed) != expected_paths:
            raise ValueError("Bootstrap Ender27 held lease set differs")
    ...
    def load_config(self, path: Path) -> dict:
        ...
        base_namespace: dict[str, object] = {
            ...
            "__name__": "__ender27_frozen_base__",
        }
...
def expected_procedure(name: str) -> dict:
    mapping = {
        "r1_control_rawresid_seed1337": (1337, "identity", None),
        "r1_tempered_grank_resid_seed1337": (
            1337,
            "tie_kept_rank_gaussian",
            0.5,
        ),
        "r1_control_rawresid_seed2027": (2027, "identity", None),
        "r1_tempered_grank_resid_seed2027": (
            2027,
            "tie_kept_rank_gaussian",
            0.5,
    ...
    try:
        model_seed, benchmark_transform, benchmark_transform_strength = mapping[name]
    except KeyError as error:
...
def validate_config(
    ...
    if procedure["benchmark_transform"] != "identity":
        ...
        expected_target_transform["benchmark_transform_strength"] = procedure[
            "benchmark_transform_strength"
        ]
    if (
...
def validate_completion(
    ...
    if (
        ...
        or payload["stage"] != "ender27-round1-training-completion"
...
def load_authority(
    ...
    expected_authority = {
        ...
        "forbidden_historical_confirmation": {
            "first_era": "0865",
            "last_era": "1021",
            "rule": "never read, score, select, tune, or report in Ender27",
        },
    ...
    if authority != expected_authority:
        raise ValueError("Ender27 discovery authority envelope differs from freeze")
    expected = {
        "full": (
...
def score_candidate(
    ...
    semantics = expected_semantics(
        ...
        procedure["benchmark_transform_strength"],
    )
