"""Governed implementation for Ender27 Round-1 evaluation."""
...
def _exact_config_pair_delta(
    ...
    control_strength = control["model"]["target_transform"].pop(
        "benchmark_transform_strength", None
    )
    challenger_mode = challenger["model"]["target_transform"].pop(
        "benchmark_transform", None
    )
    challenger_strength = challenger["model"]["target_transform"].pop(
        "benchmark_transform_strength", None
    )
    return (
        control_mode == "identity"
        and control_strength is None
        and challenger_mode == "tie_kept_rank_gaussian"
        and challenger_strength == 0.5
...
def evaluate(
    ...
    if len({record["parameter_count"] for record in records.values()}) != 1:
        raise ValueError("Ender27 parameter count differs across runs")
    ...
    aggregates = {
        ...
        "tempered_gaussian_rank_residual": procedure_aggregates(
            records, challenger_names
        ),
    }
    aggregate_decisions = aggregate_checks(
        aggregates["control"], aggregates["tempered_gaussian_rank_residual"]
    )
    challenger_checks = {
    ...
    return {
        "schema_version": 1,
        "stage": "ender27-round1-tempered-alignment",
        "state": (
            "ENDER27_ROUND2_SOURCE_GATE_ELIGIBLE"
            if passed
            else "ENDER27_NEGATIVE_NO_TEMPERED_ALIGNMENT_GAIN"
        ),
