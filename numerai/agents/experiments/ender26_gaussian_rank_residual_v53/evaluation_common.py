"""Shared, fail-closed scoring rules for frozen Ender26 Round 1.
...
EXPERIMENT_NAME = "ender26_gaussian_rank_residual_v53"
...
PAIR_NAMES = {
    "1337": ("r1_control_rawresid_seed1337", "r1_grank_resid_seed1337"),
    "2027": ("r1_control_rawresid_seed2027", "r1_grank_resid_seed2027"),
}
RECENT_BLOCKS = {
...
def expected_semantics(benchmark_transform: str) -> dict:
    target_transform = dict(BASE_TARGET_TRANSFORM)
    if benchmark_transform != "identity":
...
def verify_governed_manifest(custody: "EvaluationCustody") -> dict:
    """Reverify the one bootstrap-held envelope without reopening any path."""
    ...
    if not isinstance(manifest, dict) or set(manifest) != {
        ...
        raise ValueError("Held Ender26 source manifest schema differs")
    if manifest["schema_version"] != 1 or manifest["hash_algorithm"] != "sha256":
        raise ValueError("Held Ender26 source manifest version differs")
    if set(manifest["files"]) != _manifest_file_set(1):
        raise ValueError("Held Ender26 source manifest file set differs")
    expected_external = {
    ...
    if set(manifest["external_artifacts"]) != expected_external:
        raise ValueError("Held Ender26 external artifact set differs")
    if manifest["runtime"] != EXPECTED_RUNTIME or platform.python_version() != (
        EXPECTED_RUNTIME["python"]
    ):
        raise ValueError("Held Ender26 runtime differs")
    for package, version in EXPECTED_RUNTIME["packages"].items():
        if importlib_metadata.version(package) != version:
            raise ValueError(f"Held Ender26 package drifted: {package}")

    manifest_path = custody.repo_dir / PREFIX / "source_manifest_round1.json"
    raw_manifest = custody.read_bytes(manifest_path)
    try:
        decoded = json.loads(raw_manifest)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("Held Ender26 source manifest bytes are invalid") from error
    if decoded != manifest or raw_manifest != custody.manifest_bytes:
        raise ValueError("Held Ender26 source manifest bytes changed")
    for relative, expected in sorted(manifest["files"].items()):
        if not _valid_sha256(expected):
            raise ValueError(f"Held source digest is malformed: {relative}")
        if custody.lease(custody.repo_dir / relative).sha256() != expected:
            raise ValueError(f"Held Ender26 source changed: {relative}")
    for relative, expected in sorted(manifest["external_artifacts"].items()):
        if (
            ...
            raise ValueError(f"Held external receipt is malformed: {relative}")
        lease = custody.lease(custody.repo_dir / relative)
        if (
            lease.size_bytes() != expected["size_bytes"]
            or lease.sha256() != expected["sha256"]
        ):
            raise ValueError(f"Held Ender26 external changed: {relative}")
    return manifest
...
class DecisionReservation:
    """CREATE_NEW decision handle preserved on failure, durable on commit."""
    ...
    def __exit__(self, *_args) -> None:
        ...
        if not os.path.lexists(self.path):
            raise ValueError("Uncommitted decision reservation disappeared")
        lexical = self.path.lstat()
...
def require_frozen_python_runtime(repo_dir: Path) -> Path:
    ...
    if (
        ...
        raise ValueError(
            "Ender26 evaluation requires Python -B with an isolated "
    ...
    if prefix != Path(os.path.abspath(launch)) or not prefix.is_absolute():
        raise ValueError("Ender26 pycache_prefix differs from frozen launch state")
    try:
        prefix.relative_to(Path(os.path.abspath(repo_dir)))
    except ValueError:
        pass
    else:
        raise ValueError("Ender26 pycache_prefix must be outside the repository")
    require_plain_directory_chain(prefix)
    if next(prefix.iterdir(), None) is not None:
        raise ValueError("Ender26 pycache_prefix must be freshly empty")
    return prefix


def _manifest_file_set(_round_number: int = 1) -> set[str]:
    if _round_number != 1:
        raise ValueError("Ender26 currently defines only a Round-1 manifest")
    shared = {
    ...
    shared.update(f"{PREFIX}/configs/{name}.py" for name in ROUND1_NAMES)
    if len(shared) != 31:
        raise AssertionError("Ender26 Round-1 manifest set is not exactly 31 paths")
    return shared


class EvaluationCustody:
    """Held immutable envelope for one Ender26 Round-1 evaluation."""
    ...
    def __init__(
        ...
        *,
        ...
        self.bootstrap_leases = tuple(bootstrap_leases)
        self.leases: dict[Path, object] = {}
        self.owned_leases: list[HeldReadLease] = []

    def _hold(self, path: Path, label: str) -> HeldReadLease:
        canonical = Path(os.path.abspath(path))
        if canonical not in self.leases:
            lease = HeldReadLease(canonical, label)
            self.leases[canonical] = lease
            self.owned_leases.append(lease)
    ...
    def _acquire(self):
        if self.round_number != 1:
            raise ValueError("Unknown Ender26 evaluation round")
        ...
        if not isinstance(self.manifest, dict):
            raise ValueError("Bootstrap did not supply the held Ender26 manifest")
        if not isinstance(self.manifest_bytes, bytes) or not self.manifest_bytes:
            raise ValueError("Bootstrap did not supply held Ender26 manifest bytes")
        files = self.manifest.get("files")
        external = self.manifest.get("external_artifacts")
        if not isinstance(files, dict) or not isinstance(external, dict):
            raise ValueError("Bootstrap Ender26 manifest collections differ")
        expected_paths = {
            Path(os.path.abspath(manifest_path)),
            *(Path(os.path.abspath(self.repo_dir / path)) for path in files),
            *(Path(os.path.abspath(self.repo_dir / path)) for path in external),
        }
        borrowed = {
            Path(os.path.abspath(lease.path)): lease
            for lease in self.bootstrap_leases
        }
        if len(borrowed) != len(self.bootstrap_leases) or set(borrowed) != expected_paths:
            raise ValueError("Bootstrap Ender26 held lease set differs")
        self.leases.update(borrowed)
        verify_governed_manifest(self)
    ...
    def load_config(self, path: Path) -> dict:
        ...
        base_namespace: dict[str, object] = {
            ...
            "__name__": "__ender26_frozen_base__",
        }
...
def expected_procedure(name: str) -> dict:
    mapping = {
        "r1_control_rawresid_seed1337": (1337, "identity"),
        "r1_grank_resid_seed1337": (1337, "tie_kept_rank_gaussian"),
        "r1_control_rawresid_seed2027": (2027, "identity"),
        "r1_grank_resid_seed2027": (2027, "tie_kept_rank_gaussian"),
    }
    try:
        model_seed, benchmark_transform = mapping[name]
    except KeyError as error:
        raise ValueError(f"Unknown Ender26 Round-1 component {name}") from error
    return {
...
def validate_config(
    ...
    if not isinstance(config, dict) or set(config) != {
        ...
        raise ValueError(f"{name} config schema differs")
    ...
    if not isinstance(model, dict) or set(model) != {
        ...
        raise ValueError(f"{name} model schema differs")
    expected_target_transform = dict(BASE_TARGET_TRANSFORM)
    if procedure["benchmark_transform"] != "identity":
        expected_target_transform["benchmark_transform"] = procedure[
            "benchmark_transform"
        ]
    if (
        model.get("type") != "TorchTabularRegressor"
        or model.get("x_groups") != ["features", "era", "benchmark_models"]
        or model.get("target_transform") != expected_target_transform
    ):
...
def validate_completion(
    ...
    if (
        ...
        or payload["stage"] != "ender26-round1-training-completion"
...
def canonical_lf_json_bytes(raw: bytes, label: str) -> bytes:
    """Canonicalize one uniformly LF or CRLF JSON authority to LF bytes."""

    if not isinstance(raw, bytes) or not raw:
        raise ValueError(f"{label} authority bytes are empty")
    if raw.startswith(b"\xef\xbb\xbf"):
        raise ValueError(f"{label} authority has a forbidden UTF-8 BOM")
    has_crlf = b"\r\n" in raw
    without_crlf = raw.replace(b"\r\n", b"")
    if b"\r" in without_crlf:
        raise ValueError(f"{label} authority contains a bare CR")
    if has_crlf and b"\n" in without_crlf:
        raise ValueError(f"{label} authority mixes LF and CRLF")
    canonical = raw.replace(b"\r\n", b"\n")
    try:
        canonical.decode("utf-8")
    except UnicodeDecodeError as error:
        raise ValueError(f"{label} authority is not UTF-8") from error
    return canonical
...
def load_authority(
    ...
    expected_authority = {
        "schema_version": 1,
        "authority": "ender26-discovery-only",
        "data_version": "v5.3",
        "full": {
        ...
        "oof": {
            ...
            "era_count": 141,
            "rows": 768_362,
            "recent40_first_era": "0705",
            "recent40_last_era": "0861",
        },
        "forbidden_historical_confirmation": {
            "first_era": "0865",
            "last_era": "1021",
            "rule": "never read, score, select, tune, or report in Ender26",
        },
    ...
    if authority != expected_authority:
        raise ValueError("Ender26 discovery authority envelope differs from freeze")
    expected = {
        "full": (
    ...
    for label, (path, size, digest) in expected.items():
        ...
        if label in {"era_allowlist", "feature_columns"}:
            raw = custody.read_bytes(path) if custody else path.read_bytes()
            canonical = canonical_lf_json_bytes(raw, label)
            actual_size = len(canonical)
            actual_digest = hashlib.sha256(canonical).hexdigest()
        else:
            actual_size = custody.lease(path).size_bytes() if custody else path.stat().st_size
            actual_digest = sha256_file(path, custody)
        if actual_size != size or actual_digest != digest:
            raise ValueError(f"Semantic authority differs for {label}")
    ...
    allowlist_raw = custody.read_bytes(allowlist_path) if custody else allowlist_path.read_bytes()
    allowed = json.loads(canonical_lf_json_bytes(allowlist_raw, "era_allowlist"))
    ...
    features_raw = custody.read_bytes(features_path) if custody else features_path.read_bytes()
    features = json.loads(canonical_lf_json_bytes(features_raw, "feature_columns"))
...
def load_truth(
    ...
    if full.empty or benchmark.empty:
        raise ValueError("A discovery physical input is empty")
    ...
    for column in (TARGET, BENCHMARK):
        try:
            numeric = pd.to_numeric(truth[column], errors="raise").to_numpy(
                dtype="float64", copy=False, na_value=np.nan
            )
        except (TypeError, ValueError, OverflowError) as error:
            raise ValueError(
                f"Frozen discovery truth has malformed {column}"
            ) from error
        if not np.isfinite(numeric).all():
            raise ValueError(f"Frozen discovery truth has non-finite {column}")
        truth[column] = numeric
    return truth
...
def compute_metrics(joined, label: str = "prediction") -> dict:
    ...
    expected_eras = {f"{era:04d}" for era in range(301, 862, 4)}
    for metric_name, series in (
        ("BMC", bmc),
        ("Corr", corr),
        ("benchmark Corr", similarity),
    ):
        numeric = np.asarray(series, dtype="float64")
        if (
            len(series) != 141
            or series.index.has_duplicates
            or set(series.index) != expected_eras
            or not np.isfinite(numeric).all()
        ):
            raise ValueError(
                f"{label} {metric_name} must be finite for every exact OOF era"
            )
    if not bmc.index.equals(corr.index) or not bmc.index.equals(similarity.index):
        raise ValueError(f"{label} metric era indices differ")
...
def validate_model_diagnostics(
    ...
    *,
    ...
    if not isinstance(diagnostics, dict) or set(diagnostics) != BASE_DIAGNOSTIC_KEYS:
...
def score_candidate(
    ...
    semantics = expected_semantics(procedure["benchmark_transform"])
    ...
    if semantics_raw is None or json.loads(semantics_raw) != semantics:
    ...
    for actual, expected in zip(stored_cv["folds"], expected_folds):
        ...
        count = validate_model_diagnostics(actual["model_diagnostics"], name=name)
    ...
    validate_stored_result_schema(
        stored, config, experiment, name, joined, semantics
    )
...
def candidate_run_checks(metrics: dict, control: dict) -> dict:
    blocks = list(metrics["recent_blocks_bmc_mean"].values())
    return {
        "matched_recent40_bmc_at_least_control": (
            metrics["recent40_bmc_mean"] >= control["recent40_bmc_mean"]
        ),
        "full_bmc_retains_95pct_matched_control": (
            metrics["bmc"]["mean"] >= 0.95 * control["bmc"]["mean"]
        ),
        ...
        "corr_at_least_0_008": metrics["corr_mean"] >= 0.008,
...
def aggregate_checks(control: dict, challenger: dict) -> dict:
    return {
        "mean_full_bmc_at_least_control_plus_0_00020": (
            challenger["full_bmc"]["mean"]
            >= control["full_bmc"]["mean"] + 0.00020
        ),
        "mean_recent40_bmc_at_least_control_plus_0_00030": (
            challenger["recent40_bmc"]["mean"]
            >= control["recent40_bmc"]["mean"] + 0.00030
        ),
    }
