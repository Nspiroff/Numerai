"""Frozen shared config for Ender26 Gaussian-rank residual research."""
...
CONFIG = {
    ...
    "output": {
        "output_dir": "experiments/ender26_gaussian_rank_residual_v53",
...
def variant(
    ...
    *,
    ...
    """Return one exact matched raw- or Gaussian-rank-residual config."""
    config = deepcopy(CONFIG)
    config["output"]["results_name"] = name
    config["model"]["params"]["seed"] = model_seed
    if benchmark_transform is not None:
        config["model"]["target_transform"]["benchmark_transform"] = (
            benchmark_transform
        )
    return config
