"""Stdlib-only custody boundary for one exact Ender26 Round-1 process."""
...
PREFIX = "numerai/agents/experiments/ender26_gaussian_rank_residual_v53"
ROUND1_SEQUENCE = (
...
def _expected_manifest_files(round_number: int) -> frozenset[str]:
    if round_number != 1:
        raise ValueError("Ender26 bootstrap supports Round 1 only.")
    files = {
        *BOOTSTRAP_SOURCES,
...
def _plain_directory_chain(path: Path) -> None:
    ...
    for directory in reversed([absolute, *absolute.parents]):
        ...
        if directory.is_symlink() or bool(
            ...
            raise ValueError(
                f"Ender26 bootstrap directory is a reparse point: {directory}"
            )
        if not stat.S_ISDIR(inspected.st_mode):
            raise ValueError(
                f"Ender26 bootstrap directory is not plain: {directory}"
            )
...
def _plain_file(path: Path, label: str) -> None:
    ...
    if (
        ...
        raise ValueError(
            f"Ender26 bootstrap {label} is not a unique plain file: {path}"
        )
    ...
    def __init__(self, path: Path, label: str) -> None:
        ...
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot lease Ender26 {label}: {path}"
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    f"Leased Ender26 {label} identity is malformed or changed."
                )
...
class _DecisionReservation:
    ...
    def __enter__(self):
        ...
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot reserve Ender26 evaluation decision: {self.path}"
                ) from ctypes.WinError(ctypes.get_last_error())
            descriptor = msvcrt.open_osfhandle(
        ...
        if (
            ...
            raise ValueError("Reserved Ender26 evaluation decision is malformed.")
        return self
...
class _TrainingOutputReservations:
    ...
    def _open(path: Path, label: str):
        if os.name == "nt":
            ...
            if handle == ctypes.c_void_p(-1).value:
                raise ValueError(
                    f"Cannot reserve Ender26 {label}: {path}"
                ) from ctypes.WinError(ctypes.get_last_error())
            descriptor = msvcrt.open_osfhandle(
                int(handle), os.O_RDWR | getattr(os, "O_BINARY", 0)
            )
    ...
    def _identity(self, label: str, path: Path, stream) -> dict:
        ...
        if (
            ...
            raise ValueError(f"Reserved Ender26 {label} identity changed.")
        return {
            "path": str(path),
    ...
    def completion_identities(self) -> dict[str, dict[str, object]]:
        ...
        for label, path, stream in (
            ...
            if inspected.st_size <= 0:
                raise ValueError(f"Completed Ender26 {label} is empty.")
            digest = hashlib.sha256()
            stream.seek(0)
    ...
    def write_completion(self, payload: dict) -> bytes:
        if os.fstat(self.completion_stream.fileno()).st_size != 0:
            raise ValueError("Reserved Ender26 completion is no longer empty.")
        payload_bytes = (
            json.dumps(payload, indent=2, sort_keys=True, allow_nan=False).encode(
                "utf-8"
            )
...
def _require_launch() -> Path:
    ...
    if preloaded:
        raise ValueError(f"Ender26 governed modules were pre-imported: {preloaded}")
    ...
    if (
        ...
        raise ValueError("Ender26 bootstrap requires isolated Python -I -B -P.")
    if type(option) is not str or not option or sys.pycache_prefix != option:
        raise ValueError("Ender26 bootstrap requires exact -X pycache_prefix.")
    prefix = Path(option)
    if not prefix.is_absolute() or prefix != Path(os.path.abspath(prefix)):
        raise ValueError("Ender26 pycache prefix must be absolute and canonical.")
    try:
        prefix.relative_to(REPO_DIR)
    except ValueError:
        pass
    else:
        raise ValueError("Ender26 pycache prefix must be outside the repository.")
    _plain_directory_chain(prefix)
    if next(prefix.iterdir(), None) is not None:
        raise ValueError("Ender26 pycache prefix must be freshly empty.")
    return prefix
...
def _git(*args: str, allow_one: bool = False) -> subprocess.CompletedProcess:
    ...
    if result.returncode not in ({0, 1} if allow_one else {0}):
        raise ValueError(
            result.stderr.strip() or "Ender26 bootstrap Git verification failed."
        )
    return result
...
def _acquire(round_number: int) -> tuple[dict, bytes, tuple[_Lease, ...]]:
    ...
    try:
        ...
        if not isinstance(manifest, dict) or set(manifest) != {
            ...
            raise ValueError("Ender26 bootstrap manifest schema differs.")
        if manifest["schema_version"] != 1 or manifest["hash_algorithm"] != "sha256":
            raise ValueError("Ender26 bootstrap manifest version differs.")
        ...
        if (
            ...
            raise ValueError("Ender26 bootstrap git_head is malformed.")
        if (
            _git("rev-parse", "--verify", f"{commit}^{{commit}}").stdout.strip()
            != commit
        ):
            raise ValueError("Ender26 bootstrap git_head does not resolve.")
        if _git(
            "merge-base", "--is-ancestor", commit, "HEAD", allow_one=True
        ).returncode:
            raise ValueError("Ender26 bootstrap git_head is not an ancestor of HEAD.")
        ...
        if _git(
            ...
            *governed,
            ...
            raise ValueError("Ender26 bootstrap source is not committed and clean.")
        for relative_text, expected in sorted(files.items()):
            if (
                ...
                raise ValueError(
                    f"Ender26 bootstrap source digest is malformed: {relative_text}"
                )
            ...
            if lease.sha256() != expected:
                raise ValueError(f"Ender26 bootstrap source drifted: {relative_text}")
            if _git(
                "diff", "--quiet", commit, "--", relative_text, allow_one=True
            ).returncode:
                raise ValueError(
                    f"Ender26 bootstrap source differs from git_head: {relative_text}"
                )
        for relative_text, artifact_receipt in sorted(external.items()):
            if (
                ...
                raise ValueError(
                    f"Ender26 bootstrap external receipt differs: {relative_text}"
                )
            ...
            if (
                ...
                raise ValueError(f"Ender26 bootstrap artifact drifted: {relative_text}")
...
def _round1_output_paths(name: str) -> tuple[Path, Path, Path]:
    if name not in ROUND1_NAMES:
        raise ValueError(f"Unknown Ender26 Round-1 component: {name}")
    experiment = Path(os.path.abspath(REPO_DIR / PREFIX))
    return (
...
def _preflight_round1_sequence(name: str) -> tuple[str, ...]:
    ...
    experiment = Path(os.path.abspath(REPO_DIR / PREFIX))
    decision = experiment / "receipts/round1_bmc_alignment.json"
    if os.path.lexists(decision):
        raise ValueError("Ender26 decision destination is already consumed.")
    ...
    for predecessor in predecessors:
        for path in _round1_output_paths(predecessor):
            if not os.path.lexists(path):
                raise ValueError(
                    f"Ender26 predecessor output is missing: {path}"
                )
            _plain_directory_chain(path.parent)
            _plain_file(path, f"predecessor output {path.name}")
            if path.stat().st_size <= 0:
                raise ValueError(
                    f"Ender26 predecessor output is empty: {path}"
                )
    ...
    for pending in ROUND1_SEQUENCE[index:]:
        for path in _round1_output_paths(pending):
            if os.path.lexists(path):
                raise ValueError(
                    f"Ender26 current or future output is already consumed: {path}"
                )
    return predecessors
    ...
    try:
        ...
        for predecessor in predecessors:
            ...
            try:
                ...
                raise ValueError(
                    f"Ender26 predecessor completion is malformed: {predecessor}"
                ) from error
            if not isinstance(completion, dict) or set(completion) != {
                ...
                raise ValueError(
                    f"Ender26 predecessor completion schema differs: {predecessor}"
                )
            config_relative = f"{PREFIX}/configs/{predecessor}.py"
            if (
                completion["schema_version"] != 1
                or completion["stage"]
                != "ender26-round1-training-completion"
                ...
                raise ValueError(
                    f"Ender26 predecessor completion envelope differs: {predecessor}"
                )
            for label, path in (
                ...
                if completion["outputs"].get(label) != expected:
                    raise ValueError(
                        "Ender26 predecessor output identity differs: "
                        f"{predecessor} {label}"
                    )
        return tuple(leases)
    except BaseException:
...
def _training_main(arguments: list[str]) -> int:
    ...
    with _TrainingOutputReservations(args.config) as reservations:
        ...
        try:
            ...
            pipeline.run_ender26_training_from_bootstrap(
                config,
...
def _load_governed_module(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise ValueError(f"Cannot load governed Ender26 module: {path}")
...
def _evaluation_main(arguments: list[str]) -> int:
    ...
    expected_output = experiment / "receipts/round1_bmc_alignment.json"
    if experiment != expected_experiment:
        raise ValueError("Ender26 evaluator requires the canonical experiment.")
    if numerai_dir != Path(os.path.abspath(REPO_DIR / "numerai")):
        raise ValueError("Ender26 evaluator requires the canonical data root.")
    if output != expected_output:
        raise ValueError("Ender26 evaluator output path differs from the freeze.")
